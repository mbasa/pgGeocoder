...
--
insert into pggeocoder.boundary_s (todofuken, shikuchoson, code, geom)
...
--
insert into pggeocoder.boundary_t (todofuken, code, geom)
...
--
update pggeocoder.address_s as a set lat = st_y(b.geom), lon = st_x(b.geom), geog = b.geom::geography
  from (
...
insert into pggeocoder.address_s (todofuken, shikuchoson, lat, lon, code, geog)
  select b.todofuken, b.shikuchoson, st_y(point) as lat, st_x(point) AS lon, b.code, b.point::geography as geog
    from (select *, st_pointonsurface(geom) as point from pggeocoder.boundary_s) as b
      left join (select *, geog::geometry as geom from pggeocoder.address_s) as a on st_contains(b.geom, a.geom)
    where a.code is null;

--
-- Adjusting the Pref(Todofuken) location in address_t Table
--
update pggeocoder.address_t as a set lat = st_y(b.geom), lon = st_x(b.geom), geog = b.geom::geography
  from (
